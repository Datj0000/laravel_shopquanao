                        ...
                        <div class="card-body">
                            {{-- <div class="form-group">
                                <label>Thuộc danh mục</label>
                                  <select id="category_parent" class="form-control form-control-solid">
                            ...
                            <div class="form-group">
                                ...
                                <span class="form-text text-muted">Vui lòng điền tên danh mục sản phẩm</span>
                            </div>
                            <div class="form-group">
                                <label>Mô tả danh mục:</label>
                                <textarea placeholder="Mô tả danh mục" id="category_desc"
                                    class="form-control form-control-solid" cols="30" rows="10"></textarea>
                                <span class="form-text text-muted">Vui lòng mô tả danh mục sản phẩm</span>
                            </div>
                            <div class="form-group">
                ...
                <div class="modal-body">
                    <form class="form">
                        ...
                        <div class="card-body">
                            ...
                            {{-- <div class="form-group">
                                <label>Thuộc danh mục</label>
                                  <select id="edit_category_parent" class="form-control form-control-solid">
                                    <option value="0">---Danh mục cha---</option>
                                    @foreach($category as $key => $val)
                                       <option value="{{$val->category_id}}">{{$val->category_name}}</option>
                                    @endforeach
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
            ...
            if (category_name == "" || category_desc == "") {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        slug_category_product: category_slug,
                        _token: _token,
                    },
        ...
        $(document).on('click', '#update_category', function(e) {
            var category_parent = $('#edit_category_parent').val();
            var category_product_id = $('#edit_category_id').val();
            var category_product_name = $('#edit_category_name').val();
            var category_product_desc = $('#edit_category_desc').val();
            var category_product_slug = ChangeToSlug(category_product_name);
            var _token = $('input[name="_token"]').val();
            if (category_product_name == "" || category_product_desc == "") {
                Swal.fire("Thất bại", "Vui lòng không để trống các thông tin!", "error");
            } else {
                $.ajax({
                    url: 'update-category-product/' + category_product_id,
                    method: 'POST',
                    data: {
                        category_parent: category_parent,
                        category_product_id: category_product_id,
                        category_product_name: category_product_name,
                        category_product_desc: category_product_desc,
                        slug_category_product: category_product_slug,
                        _token: _token,
                    },
                    success: function(data) {
                        if (data == 1) {
        ...
        $(document).on('click', '.edit', function(e) {
            e.preventDefault();
            var category_product_id = $(this).data('id_category');
            $.ajax({
                url: 'edit-category-product/' + category_product_id,
                method: 'GET',
        ...
        $(document).on('click', '.activecat', function(e) {
            ...
            var category_product_id = $(this).data('id_category');
            $.ajax({
                url: 'active-category-product/' + category_product_id,
                method: 'GET',
                success: function(response) {
                    Swal.fire({
                        icon: "success",
        ...
        $(document).on('click', '.unactivecat', function(e) {
            ...
            var category_product_id = $(this).data('id_category');
            $.ajax({
                url: 'unactive-category-product/' + category_product_id,
                method: 'GET',
                success: function(response) {
                    Swal.fire({
                        icon: "success",
        ...
        $(document).on('click', '.delete', function(e) {
            ...
            Swal.fire({
                ...
                .then(function(result) {
                    if (result.value) {
                        $.ajax({
                            url: 'delete-category-product/' + category_product_id,
                            method: 'GET',
                            success: function(feedback) {
                                if (feedback == 1) {
                                    ...
                                    Swal.fire("Thất bại", "Danh mục đang có sản phẩm!",
                                        "error");
                                }
                            },
                        });
